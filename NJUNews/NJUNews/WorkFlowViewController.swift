...
class WorkFlowViewController: UIViewController {
    ...
    func downloadHTML()
    {
        guard let url = URL(string: "https://jw.nju.edu.cn/ggtz/list.htm") else {return}
        do
        {
            // content of url
            ...
            for element in try document.select("ul")
            {
                if try element.className() == "news_list list2"
                {
                    for list in try element.select("li")
                    {
                        ...
                        WKitems.append(Item(title: title, date: date, html: html))
                    }
                }
            }
        }
...
extension WorkFlowViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return WKitems.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "WorkFlowCell") as! WorkFlowTableViewCell
        cell.textLabel?.text = WKitems[indexPath.row].title
        cell.detailTextLabel?.text = WKitems[indexPath.row].date
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        // 找到被选cell
        guard let selectedCell = sender as? WorkFlowTableViewCell else{
        ...
        destViewController.html = WKitems[indexPath].html
    }
}
